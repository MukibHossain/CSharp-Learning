...
//This program models three employee types (Full-Time, Part-Time, Contract) that inherit
//from a common Employee base class. Each derived class overrides CalculateSalary() to apply
//its own salary formula, and the base class also defines overloaded versions of CalculateSalary()
//that accept extra parameters (bonus / hourly rate & hours / contract amount & deduction). Design Summary
...
//A List<Employee> holds all employee objects; CalculateSalary() is invoked via the
//base-class reference, demonstrating runtime polymorphism.
